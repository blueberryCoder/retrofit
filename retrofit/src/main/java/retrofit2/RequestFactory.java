...
final class RequestFactory {
    ...
    static final class Builder {
        ...
        final Method method;
        final Annotation[] methodAnnotations;  //函数的注解
        final Annotation[][] parameterAnnotationsArray; // 参数的注解
        final Type[] parameterTypes;  // 参数的类型
        ...
        String httpMethod;
        boolean hasBody;
        boolean isFormEncoded;              // 表单
        boolean isMultipart;                // multipart
        String relativeUrl;                 // 相对路径，不自动截取，xxx/xxx?xxx=xx
        Headers headers;                    // 请求头
        MediaType contentType;              // Content-type
        Set<String> relativeUrlParamNames;  // url 参数
        ParameterHandler<?>[] parameterHandlers;  // 参数处理器
        ...
        RequestFactory build() {
            ...
            for (int p = 0; p < parameterCount; p++) {
                ...
                if (Utils.hasUnresolvableType(parameterType)) {  // 不能存在不确定类型
        ...
        private void parseMethodAnnotation(Annotation annotation) {
            if (annotation instanceof DELETE) {
                ...
                parseHttpMethodAndPath(http.method(), http.path(), http.hasBody());  // 解析url
        ...
        private void parseHttpMethodAndPath(String httpMethod, String value, boolean hasBody) {
            ...
            if (question != -1 && question < value.length() - 1) {
                ...
                if (queryParamMatcher.find()) { // query 和 path 传参不能同时出现
        ...
        private Headers parseHeaders(String[] headers) {
            ...
            for (String header : headers) {
                ...
                if ("Content-Type".equalsIgnoreCase(headerName)) {  // Content-Type 一定要有值
        ...
        private ParameterHandler<?> parseParameterAnnotation(
            ...
            if (annotation instanceof Url) {
                ...
                gotQuery = true;
                if (Iterable.class.isAssignableFrom(rawParameterType)) {  // 如果属于Iterable,必须要有范型参数
                    ...
                    Class<?> arrayComponentType = boxIfPrimitive(rawParameterType.getComponentType()); // 如果是基本类型，进行装箱
                    Converter<?, String> converter =
